...
```python
# agents/intention/recognizer.py
"""
意图识别模块
...
class IntentionType(Enum):
    TROUBLESHOOT = "troubleshoot"        # 设备故障需要诊断
    QUERY_KNOWLEDGE = "query_knowledge"  # 查询设备知识
    SEEK_GUIDANCE = "seek_guidance"      # 需要维修步骤
...
class IntentionRecognizer:
    ...
    使用 qwen-turbo 轻量级模型，支持快速识别和降级策略。
    """
    ...
    async def recognize(self, message: str) -> IntentionResult:
        """执行意图识别"""
        try:
            return await self._llm_recognize(message)
        except Exception:
            return self._keyword_fallback(message)
    
    async def _llm_recognize(self, message: str) -> IntentionResult:
        """LLM意图识别（优先）"""
        ...
    
    def _keyword_fallback(self, message: str) -> IntentionResult:
        """关键词兜底（降级方案）"""
        ...
```
